...
function embaralharNumeros() {
    ...
    // Seleciona uma pergunta que não foi usada
    numPergunta = data[0]['perguntas'+ nivel][numAleatorio[Math.floor(Math.random() * numAleatorio.length)]];
    while (numUsados.includes(numPergunta)) {
        numPergunta = data[0]['perguntas'+ nivel][numAleatorio[Math.floor(Math.random() * numAleatorio.length)]];
    }
...
function endGame() {
    ...
    buttonContinue.textContent = 'Voltar ao Início';
    ...
    resultado.innerHTML = `<span>Resultado: <br>${msg}</span>`;
